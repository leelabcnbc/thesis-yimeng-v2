...
def main():
    script_dict = dict()
    for param_dict in explored_models_20200801_gaya_generator():
        assert param_dict['dataset_prefix'] == 'gaya'
        assert param_dict['model_prefix'] == 'maskcnn_polished_with_rcnn_k_bl'
        assert param_dict['rcnn_acc_type'] in {'cummean', 'cummean_last'}
    ...
    utils.submit(
        ...
        dirname_relative='scripts+gaya_refactored+maskcnn_polished_with_rcnn_k_bl+20200801'
    )
