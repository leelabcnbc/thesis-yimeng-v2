...
from thesis_v2.configs.model.maskcnn_polished_with_rcnn_k_bl import (
    explored_models_20200802_gaya_generator,
    script_keygen
)
...
def main():
    script_dict = dict()
    for param_dict in explored_models_20200802_gaya_generator(contain_model_prefix=True):
        assert param_dict['dataset_prefix'] == 'gaya'
        assert param_dict['model_prefix'] == 'maskcnn_polished_with_rcnn_k_bl'
        assert param_dict['rcnn_acc_type'] in {'instant', 'last'}
        assert len(param_dict) == 26
        key_this = script_keygen(**param_dict)

        # should be 26 keys in `call_script`
    ...
    utils.submit(
        ...
        dirname_relative='scripts+gaya_refactored+maskcnn_polished_with_rcnn_k_bl+20200802'
    )
