...
# )
from thesis_v2.data.prepared.crcns_pvc8 import (
    get_idx_sets_natural_grouped,
    get_idx_sets_legacy,
    natural_data_neural,
)
...
def master(*,
    ...
    neural_all = natural_data_neural(window_size='large', read_only=True, scale=1/50)

    num_neuron = neural_all.shape[1]
    assert neural_all.dtype == np.float64

    if shuffle_type == 'legacy':
        idx_stuff = get_idx_sets_legacy(540, split_seed)[:3]
    elif shuffle_type == 'paired':
        idx_stuff = get_idx_sets_natural_grouped(split_seed, return_dict=False)[:3]
    else:
    ...
    with TemporaryDirectory(dir=environ.get('JOBLIB_TEMP_FOLDER', None)) as d:
        ...
        # TODO change `large` to `medium` if you want to test other scales.
        with h5py.File(file_to_save_feature, 'r') as f:
            x_all = f['/'.join(['large', suffix])][...]
