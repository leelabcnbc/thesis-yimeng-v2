...
from thesis_v2.configs.model.maskcnn_polished_with_rcnn_k_bl import (
    multipath_models_8k_v1_generator,
    script_keygen
)
...
def main():
    script_dict = dict()
    for param_dict in multipath_models_8k_v1_generator(False):
        key_this = script_keygen(**param_dict)
        assert len(param_dict) == 28
        param_dict = {k: v for k, v in param_dict.items() if k not in {'dataset_prefix', 'model_prefix'}}
    ...
    utils.submit(
        ...
        dirname_relative='scripts+yuanyuan_8k_a_3day+maskcnn_polished_with_rcnn_k_bl+20220803'
    )
