...
def master(*,
           ...
           dataset_prefix: str = 'yuanyuan_8k_v2d',
    ...
    v2d_ind = np.array([0, 1, 2, 3, 4, 5, 6, 7, 15, 16, 17, 19, 21, 24, 27, 
                        28, 29, 30, 31, 32, 33, 41, 42, 43, 46, 52, 53, 55, 
                        56, 57, 58, 59, 66, 68, 69, 76, 77])
    datasets = {
        'X_train': datasets[0][train_keep_slice].astype(np.float32),
        'y_train': datasets[1][train_keep_slice, v2d_ind],
        'X_val': datasets[2].astype(np.float32),
        'y_val': datasets[3][:, v2d_ind],
        'X_test': datasets[4].astype(np.float32),
        'y_test': datasets[5][:, v2d_ind],
    }
