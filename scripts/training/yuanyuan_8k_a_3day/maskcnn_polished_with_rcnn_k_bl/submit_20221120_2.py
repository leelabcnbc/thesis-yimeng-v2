...
from thesis_v2.configs.model.maskcnn_polished_with_rcnn_k_bl import (
    explored_models_20221117_generator,
    explored_models_20221120_generator,
    script_keygen
)
...
def main():
    script_dict = dict()
    for param_dict in chain(
        explored_models_20221117_generator(False),
        explored_models_20221120_generator(False)
    ):
        key_this = script_keygen(**param_dict)
        assert len(param_dict) == 26
        param_dict = {k: v for k, v in param_dict.items() if k not in {'model_prefix', 'dataset_prefix'}}
    ...
    utils.submit(
        ...
        dirname_relative='scripts+yuanyuan_8k_a_3day+maskcnn_polished_with_rcnn_k_bl+20221120_2'
    )
