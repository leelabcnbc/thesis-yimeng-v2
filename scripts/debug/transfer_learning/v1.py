...
from thesis_v1 import dir_dict, join

from thesis_v1.data.prepared import split_stuff
from thesis_v1.data.prepared.yuanyuan_8k import (
    get_neural_data, get_indices
)

from thesis_v1.models.transfer_learning.builder import (gen_transfer_learner,
                                                        load_modules)

from thesis_v1.training.utils import get_memmapped_array
...
from thesis_v1.models.utils.transfer_learning.training import (train_one,
                                                               partial)

from thesis_v1.models.utils.transfer_learning.opt import (
...
def handle_one_case(x_train, y_train, x_val, y_val, x_test, y_test, act_fn,
    ...
    dataset_this = (
        x_train,
        y_train[:, neuron_idx:neuron_idx + 1],
        x_val,
        y_val[:, neuron_idx:neuron_idx + 1],
        x_test,
        y_test[:, neuron_idx:neuron_idx + 1],
    )
    ...
    opt_config_partial = partial(get_transfer_learning_opt_config,
                                 sparse=float(sparse))
    #
    res = train_one(gen_cnn_partial, opt_config_partial,
                    dataset_this,
                    # note this gets saved under v1 folder...
                    # but it should not matter.
                    key=f'debug/transfer_learning/v1_code/{note}/{act_fn}/{sparse}/{neuron_idx}',
                    legacy_corr=True,
