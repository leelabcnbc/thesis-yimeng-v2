...
def collect_one_setting(
        *,
        ...
        keygen,
        param,
        # this way, I can ignore cases where experiments fail to run.
        num_neuron=None,
):
    ...
    try:
        ...
        for sparse_idx, sparse_this in enumerate(sparse_list):
            ...
            if not exists(summary_file):
                ...
                np.all(np.isfinite(val_this)) and np.all(
                    np.isfinite(test_this))
        ...
        # not complete.
        return {
            'test_best': np.full((num_neuron,), np.nan,
                                 dtype=np.float64),
            # I should filter out these -1's when calculating
            # argmax statistics.
            'argmax': np.full((num_neuron,), -1, dtype=np.int64),
        }
