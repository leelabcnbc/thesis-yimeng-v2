...
from ....feature_extraction.extraction import Lambda
...
class PcConvBp(nn.Module):
    def __init__(self, inchan, outchan, kernel_size=3, stride=1, padding=1, cls=0, bias=False):
        ...
        self.resp_init = Lambda()
        self.resp_loop = Lambda()
...
class PredNetBpE(nn.Module):
    def __init__(self, num_classes=1000, cls=0, Tied = False):
        ...
        # add this to support max pooling feature extraction.
        self.maxpool_dict = nn.ModuleDict({str(i): Lambda() for i, v in enumerate(self.maxpool) if v})
    ...
    def forward(self, x):
        ...
        for i in range(self.nlays-1):
            ...
            # this is bad really, maxpool idx is one offset from self.PcConvs
            if self.maxpool[i]:
                x = self.maxpool2d(x)
                # register.
                x = self.maxpool_dict[str(i)](x)
